...
#define DCDC_CURRENT_OFFSET_VOLTAGE 1.1f // V TODO: Why is this 1 and not 0.5?
#define DCDC_CURRENT_SENSITIVITY 0.200f // mV/A
#define DCDC_CURRENT_RTOP 10000.0
#define DCDC_CURRENT_RBOT 20000.0
#define DCDC_CURRENT_DIVIDER (DCDC_CURRENT_RBOT/(DCDC_CURRENT_RBOT+DCDC_CURRENT_RTOP))
#define DCDC_CURRENT_REF_ADC (DCDC_CURRENT_OFFSET_VOLTAGE * DCDC_CURRENT_DIVIDER / ADC_FACTOR)
float IO_GET_DCDC_CURRENT(void){
    return ((float)ADC_GetValue(DCDC_OUTPUT_CURRENT_AI) - DCDC_CURRENT_REF_ADC) * (1.0f / DCDC_CURRENT_DIVIDER) / DCDC_CURRENT_SENSITIVITY * ADC_FACTOR;
}

#define EV_CHARGER_CURRENT_REF_VOLTAGE 2.5
#define EV_CHARGER_CURRENT_SENSITIVITY 0.02f // mV/A
...
#define EV_CHARGER_CURRENT_REF_ADC (EV_CHARGER_CURRENT_REF_VOLTAGE * EV_CHARGER_CURRENT_DIVIDER / ADC_FACTOR)
#define EV_CHARGER_CURRENT_SCALE ((1.0f / EV_CHARGER_CURRENT_DIVIDER) / EV_CHARGER_CURRENT_SENSITIVITY * ADC_FACTOR)

static int16_t EV_CHARGER_CALIBRATION = 0;
void IO_SET_EV_CHARGER_CALIBRATION(void) {
    uint16_t adc_value = ADC_GetValue(EV_CHARGER_CURRENT_AI);
    EV_CHARGER_CALIBRATION = (int16_t)EV_CHARGER_CURRENT_REF_ADC - (int16_t)adc_value;
}
float IO_GET_EV_CHARGER_CURRENT(void){
    return ((float)ADC_GetValue(EV_CHARGER_CURRENT_AI) - EV_CHARGER_CURRENT_REF_ADC + EV_CHARGER_CALIBRATION) * EV_CHARGER_CURRENT_SCALE;
}
