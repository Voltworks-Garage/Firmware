...
#define SPEED_ARC_SIZE          LV_VER_RES / 1.25
#define SPEED_ARC_WIDTH         35
#define SPEED_MAX               100

// Layout - Battery Bar (right side, same as home screen)
#define BATTERY_BAR_WIDTH       LV_HOR_RES / 12
#define BATTERY_BAR_HEIGHT      LV_VER_RES - MARGIN_TOP - MARGIN_BOTTOM - LV_VER_RES / 4  // Leave space for percentage label
...
#define POWER_BAR_WIDTH         LV_HOR_RES - 8*MARGIN_LEFT - MARGIN_RIGHT - LV_HOR_RES / 4  // Leave space for power label
#define POWER_BAR_HEIGHT        LV_HOR_RES / 24
...
void ScreenRunning_Create(void) {
    ...
    lv_obj_add_style(label_speed, &style_label_extra_large_green, 0);
    ...
    lv_obj_align_to(label_speed, arc_speed, LV_ALIGN_CENTER, 0, 0);
    ...
    lv_obj_align_to(label_speed_unit, label_speed, LV_ALIGN_OUT_BOTTOM_MID, 0, 0);
    ...
    lv_obj_align_to(label_battery_percent, bar_battery, LV_ALIGN_OUT_BOTTOM_MID, 0, 20);
    ...
    lv_obj_align(bar_power, LV_ALIGN_BOTTOM_MID, 0, 0);
    ...
    lv_obj_align_to(label_power, bar_power, LV_ALIGN_OUT_LEFT_MID, 0, 0);
    ...
    lv_obj_add_style(label_light_low, &style_label_large_gray, 0);
    ...
    lv_obj_add_style(label_light_high, &style_label_large_gray, 0);
    lv_obj_align_to(label_light_high, label_light_low, LV_ALIGN_OUT_BOTTOM_LEFT, 0, MARGIN_TOP);

    // Hazard
    label_hazard = lv_label_create(screen);
    lv_label_set_text(label_hazard, LV_SYMBOL_WARNING);
    lv_obj_add_style(label_hazard, &style_label_large_gray, 0);
    lv_obj_align_to(label_hazard, label_light_high, LV_ALIGN_OUT_BOTTOM_LEFT, 0, MARGIN_TOP);

    // Bluetooth
    label_bluetooth = lv_label_create(screen);
    lv_label_set_text(label_bluetooth, LV_SYMBOL_BLUETOOTH);
    lv_obj_add_style(label_bluetooth, &style_label_large_gray, 0);
    lv_obj_align_to(label_bluetooth, label_hazard, LV_ALIGN_OUT_BOTTOM_LEFT, 0, MARGIN_TOP);
...
void ScreenRunning_Update(void) {
    ...
    uint32_t thisTime = pdTICKS_TO_MS(xTaskGetTickCount());
    if(thisTime - lastTime >= 100){
        tempSpeed++;
        if(tempSpeed>99) tempSpeed = 0;
        lv_arc_set_value(arc_speed, tempSpeed);
        lv_label_set_text_fmt(label_speed, "%d", tempSpeed);
        tempPower += 10;
        if(tempPower>POWER_MAX_KW*10) tempPower = 0;
        lv_label_set_text_fmt(label_power, "%d.%d kW", GET_WHOLE(tempPower), GET_FRAC(tempPower));
        lv_bar_set_value(bar_power, tempPower, LV_ANIM_ON);
        lastTime = thisTime;
    }
