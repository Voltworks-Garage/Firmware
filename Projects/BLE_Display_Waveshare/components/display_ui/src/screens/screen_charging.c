...
static int cached_voltage_x10 = -1;
static uint16_t cached_cell_voltages[NUM_CELLS] = {0};

// Use LVGL's raw scroll function to correct position without firing
// SCROLL_BEGIN/SCROLL_END events that disrupt the active drag gesture.
extern lv_result_t lv_obj_scroll_by_raw(lv_obj_t * obj, int32_t x, int32_t y);

// Workaround for LVGL v9 elastic_diff bug: non-elastic clamping fails for
// negative scroll diffs, allowing 1-frame overshoot + animated snap-back.
// This callback immediately corrects any overshoot to prevent visible bounce.
static void scroll_clamp_cb(lv_event_t * e) {
    static bool in_clamp = false;
    if(in_clamp) return;
    ...
    if(sb < 0) {
        in_clamp = true;
        lv_obj_scroll_by_raw(obj, 0, -sb);
        in_clamp = false;
    }
    else if(st < 0) {
        in_clamp = true;
        lv_obj_scroll_by_raw(obj, 0, st);
        in_clamp = false;
    }
}
...
void ScreenCharging_Create(void) {
    ...
    lv_obj_add_style(label_title, &style_label_large_white, 0);
    lv_obj_set_style_align(label_title, LV_ALIGN_TOP_MID, 0);

    // SOC arc (positioned at original location)
    arc_soc = lv_arc_create(screen);
    lv_obj_set_size(arc_soc, LV_VER_RES/2, LV_VER_RES/2);
    ...
    lv_obj_set_style_align(arc_soc, LV_ALIGN_CENTER, 0);
    lv_obj_set_style_arc_color(arc_soc, lv_color_hex(COLOR_BATTERY_GREEN), LV_PART_INDICATOR);
    lv_obj_set_style_arc_width(arc_soc, 30, LV_PART_INDICATOR);
    lv_obj_set_style_arc_width(arc_soc, 30, LV_PART_MAIN);
    ...
    lv_obj_align_to(label_voltage, arc_soc, LV_ALIGN_OUT_BOTTOM_MID, 0, -10);
    ...
    lv_obj_align_to(label_status, label_voltage, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
    ...
    lv_obj_set_style_align(divider, LV_ALIGN_BOTTOM_MID, 0);
    ...
    // Register scroll clamp callback only once (screen persists across transitions)
    static bool scroll_clamp_registered = false;
    if(!scroll_clamp_registered) {
        lv_obj_add_event_cb(screen, scroll_clamp_cb, LV_EVENT_SCROLL, NULL);
        scroll_clamp_registered = true;
    }
    ...
    const int grid_start_y = LV_VER_RES;  // Moved up to give more space
...
void ScreenCharging_Destroy(void) {
    ...
    // Reset cached values
    cached_soc_int = -1;
    cached_voltage_x10 = -1;
    for (int i = 0; i < NUM_CELLS; i++) {
        cached_cell_voltages[i] = 0;
    }
}

void ScreenCharging_Update(void) {
    // Update SOC (only if changed)
    ...
    // Update pack voltage (only if changed)
    if (label_voltage != NULL && !CAN_bms_status_checkDataIsStale()) {
        ...
        if (voltage_x10 != cached_voltage_x10) {
            lv_label_set_text_fmt(label_voltage, "Pack: %d.%dV",
                                  GET_WHOLE(voltage_x10), GET_FRAC(voltage_x10));
            cached_voltage_x10 = voltage_x10;
        }
    }

    // Update individual cell voltages (only if changed)
    if (!CAN_bms_cell_voltages_checkDataIsStale()) {
        ...
        for (int i = 0; i < NUM_CELLS; i++) {
            if (label_cells[i] != NULL) {
                ...
                // Only update if voltage changed
                if (cell_voltage_mv != cached_cell_voltages[i]) {
                    ...
                    cached_cell_voltages[i] = cell_voltage_mv;
                }
            }
        }
    }
