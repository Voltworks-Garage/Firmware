...
static adc_digi_pattern_config_t adc_patterns[] = {
    { .atten = ADC_ATTEN_DB_12, .channel = X_ADC_PLUS, .unit = ADC_UNIT, .bit_width = SOC_ADC_DIGI_MAX_BITWIDTH },
    { .atten = ADC_ATTEN_DB_12, .channel = Y_ADC_PLUS, .unit = ADC_UNIT, .bit_width = SOC_ADC_DIGI_MAX_BITWIDTH },
};
...
void Touch_DeInit(void) {
    // Stop ADC sampling
    // Note: In ESP-IDF 6.0, adc_continuous_stop/deinit must be called from same task context
    // that called start/init to avoid mutex assertion failures.
    if (adc_handle != NULL) {
        // Skip stop/deinit to avoid cross-task mutex issues
        // The deep sleep will reset everything anyway
        adc_handle = NULL;
    }
...
void Touch_Run_1ms(void) {
    ...
    if(adc_handle == NULL) {
        // ADC not initialized (probably deinitialized for sleep)
        return;
    }
