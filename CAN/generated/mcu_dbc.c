...
void CAN_mcu_status_J1772_controller_fault_set(uint16_t J1772_controller_fault){
	uint16_t data_scaled = J1772_controller_fault * 1.0;
...
void CAN_mcu_status_fan_current_set(uint16_t fan_current){
	uint16_t data_scaled = fan_current * 1.0;
	// Set 12-bit signal at bit offset 3
	CAN_mcu_status_payloads[2].word0 &= ~0x7FF8;
	CAN_mcu_status_payloads[2].word0 |= (data_scaled << 3) & 0x7FF8;
}
void CAN_mcu_status_pump_current_set(uint16_t pump_current){
	uint16_t data_scaled = pump_current * 1.0;
	// Set 12-bit signal at bit offset 15
	CAN_mcu_status_payloads[2].word0 &= ~0x8000;
	CAN_mcu_status_payloads[2].word0 |= (data_scaled << 15) & 0x8000;
	CAN_mcu_status_payloads[2].word1 &= ~0x07FF;
	CAN_mcu_status_payloads[2].word1 |= (data_scaled >> 1) & 0x07FF;
}
void CAN_mcu_status_taillight_current_set(uint16_t taillight_current){
	uint16_t data_scaled = taillight_current * 1.0;
	// Set 12-bit signal at bit offset 27
	CAN_mcu_status_payloads[2].word1 &= ~0xF800;
	CAN_mcu_status_payloads[2].word1 |= (data_scaled << 11) & 0xF800;
	CAN_mcu_status_payloads[2].word2 &= ~0x007F;
	CAN_mcu_status_payloads[2].word2 |= (data_scaled >> 5) & 0x007F;
}
void CAN_mcu_status_brakelight_current_set(uint16_t brakelight_current){
	uint16_t data_scaled = brakelight_current * 1.0;
	// Set 12-bit signal at bit offset 39
	CAN_mcu_status_payloads[2].word2 &= ~0xFF80;
	CAN_mcu_status_payloads[2].word2 |= (data_scaled << 7) & 0xFF80;
	CAN_mcu_status_payloads[2].word3 &= ~0x0007;
	CAN_mcu_status_payloads[2].word3 |= (data_scaled >> 9) & 0x0007;
}
void CAN_mcu_status_lowbeam_current_set(uint16_t lowbeam_current){
	uint16_t data_scaled = lowbeam_current * 1.0;
	// Set 12-bit signal at bit offset 51
	CAN_mcu_status_payloads[2].word3 &= ~0x7FF8;
	CAN_mcu_status_payloads[2].word3 |= (data_scaled << 3) & 0x7FF8;
}
void CAN_mcu_status_highbeam_current_set(uint16_t highbeam_current){
	uint16_t data_scaled = highbeam_current * 1.0;
	// Set 12-bit signal at bit offset 3
	CAN_mcu_status_payloads[3].word0 &= ~0x7FF8;
	CAN_mcu_status_payloads[3].word0 |= (data_scaled << 3) & 0x7FF8;
}
void CAN_mcu_status_horn_current_set(uint16_t horn_current){
	uint16_t data_scaled = horn_current * 1.0;
	// Set 12-bit signal at bit offset 15
	CAN_mcu_status_payloads[3].word0 &= ~0x8000;
	CAN_mcu_status_payloads[3].word0 |= (data_scaled << 15) & 0x8000;
	CAN_mcu_status_payloads[3].word1 &= ~0x07FF;
	CAN_mcu_status_payloads[3].word1 |= (data_scaled >> 1) & 0x07FF;
}
void CAN_mcu_status_aux_port_current_set(uint16_t aux_port_current){
	uint16_t data_scaled = aux_port_current * 1.0;
	// Set 12-bit signal at bit offset 27
	CAN_mcu_status_payloads[3].word1 &= ~0xF800;
	CAN_mcu_status_payloads[3].word1 |= (data_scaled << 11) & 0xF800;
	CAN_mcu_status_payloads[3].word2 &= ~0x007F;
	CAN_mcu_status_payloads[3].word2 |= (data_scaled >> 5) & 0x007F;
}
void CAN_mcu_status_heated_grips_current_set(uint16_t heated_grips_current){
	uint16_t data_scaled = heated_grips_current * 1.0;
	// Set 12-bit signal at bit offset 39
	CAN_mcu_status_payloads[3].word2 &= ~0xFF80;
	CAN_mcu_status_payloads[3].word2 |= (data_scaled << 7) & 0xFF80;
	CAN_mcu_status_payloads[3].word3 &= ~0x0007;
	CAN_mcu_status_payloads[3].word3 |= (data_scaled >> 9) & 0x0007;
}
void CAN_mcu_status_heated_seat_current_set(uint16_t heated_seat_current){
	uint16_t data_scaled = heated_seat_current * 1.0;
	// Set 12-bit signal at bit offset 51
	CAN_mcu_status_payloads[3].word3 &= ~0x7FF8;
	CAN_mcu_status_payloads[3].word3 |= (data_scaled << 3) & 0x7FF8;
}
void CAN_mcu_status_charge_controller_current_set(uint16_t charge_controller_current){
	uint16_t data_scaled = charge_controller_current * 1.0;
	// Set 12-bit signal at bit offset 3
	CAN_mcu_status_payloads[4].word0 &= ~0x7FF8;
	CAN_mcu_status_payloads[4].word0 |= (data_scaled << 3) & 0x7FF8;
}
void CAN_mcu_status_motor_controller_current_set(uint16_t motor_controller_current){
	uint16_t data_scaled = motor_controller_current * 1.0;
	// Set 12-bit signal at bit offset 15
	CAN_mcu_status_payloads[4].word0 &= ~0x8000;
	CAN_mcu_status_payloads[4].word0 |= (data_scaled << 15) & 0x8000;
	CAN_mcu_status_payloads[4].word1 &= ~0x07FF;
	CAN_mcu_status_payloads[4].word1 |= (data_scaled >> 1) & 0x07FF;
}
void CAN_mcu_status_bms_controller_current_set(uint16_t bms_controller_current){
	uint16_t data_scaled = bms_controller_current * 1.0;
	// Set 12-bit signal at bit offset 27
	CAN_mcu_status_payloads[4].word1 &= ~0xF800;
	CAN_mcu_status_payloads[4].word1 |= (data_scaled << 11) & 0xF800;
	CAN_mcu_status_payloads[4].word2 &= ~0x007F;
	CAN_mcu_status_payloads[4].word2 |= (data_scaled >> 5) & 0x007F;
}
void CAN_mcu_status_J1772_controller_current_set(uint16_t J1772_controller_current){
	uint16_t data_scaled = J1772_controller_current * 1.0;
	// Set 12-bit signal at bit offset 39
	CAN_mcu_status_payloads[4].word2 &= ~0xFF80;
	CAN_mcu_status_payloads[4].word2 |= (data_scaled << 7) & 0xFF80;
	CAN_mcu_status_payloads[4].word3 &= ~0x0007;
	CAN_mcu_status_payloads[4].word3 |= (data_scaled >> 9) & 0x0007;
}
...
void CAN_mcu_mcu_debug_task_1000ms_cpu_percent_set(float task_1000ms_cpu_percent){
	uint16_t data_scaled = (uint16_t)(task_1000ms_cpu_percent * 1.0f + 0.5f);
	// Set 8-bit signal at bit offset 26
 ...
 * bms NODE MESSAGES
 */
static CAN_payload_S CAN_bms_status_mux_payloads[5] __attribute__((aligned(sizeof(CAN_payload_S))));
static volatile uint8_t CAN_bms_status_mux_status[5] = {0};
static CAN_message_S CAN_bms_status_mux[5];
...
#define CAN_BMS_STATUS_M3_CHARGE_ALLOWED_RANGE 1
...
uint8_t CAN_bms_status_checkMuxUnread(uint8_t mux_index){
	...
	return CAN_checkDataIsUnread(&CAN_bms_status_mux[mux_index]);
}
uint8_t CAN_bms_status_checkMuxStale(uint8_t mux_index){
	if (mux_index >= 5) return 1;
	return CAN_checkDataIsStale(&CAN_bms_status_mux[mux_index], 20);
}
...
uint16_t CAN_bms_status_bms_state_get(void){
	...
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word0 & 0x0078) >> 3) << 0;
	return (data * 1.0) + 0;
}
float CAN_bms_status_pack_voltage_get(void){
	// Extract 16-bit signal at bit offset 7
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word0 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word1 & 0x007F) >> 0) << 9;
	return (data * 0.01) + 0;
}
float CAN_bms_status_pack_current_get(void){
	// Extract 16-bit signal at bit offset 23
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word1 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word2 & 0x007F) >> 0) << 9;
	return (data * 0.01) + -320;
}
float CAN_bms_status_soc_percent_get(void){
	// Extract 8-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word2 & 0x7F80) >> 7) << 0;
	return (data * 0.5) + 0;
}
float CAN_bms_status_pack_temp_min_get(void){
	// Extract 8-bit signal at bit offset 47
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word2 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word3 & 0x007F) >> 0) << 1;
	return (data * 1.0) + -40;
}
float CAN_bms_status_pack_temp_max_get(void){
	// Extract 8-bit signal at bit offset 55
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[0].word3 & 0x7F80) >> 7) << 0;
	return (data * 1.0) + -40;
}
float CAN_bms_status_stack_voltage_1_get(void){
	// Extract 16-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[1].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[1].word1 & 0x0007) >> 0) << 13;
	return (data * 0.001) + 0;
}
float CAN_bms_status_stack_voltage_2_get(void){
	// Extract 16-bit signal at bit offset 19
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[1].word1 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[1].word2 & 0x0007) >> 0) << 13;
	return (data * 0.001) + 0;
}
float CAN_bms_status_pack_voltage_sum_of_stacks_get(void){
	// Extract 18-bit signal at bit offset 35
	uint32_t data = 0;
	data |= (uint32_t)((CAN_bms_status_mux_payloads[1].word2 & 0xFFF8) >> 3) << 0;
	data |= (uint32_t)((CAN_bms_status_mux_payloads[1].word3 & 0x001F) >> 0) << 13;
	return (data * 0.001) + 0;
}
uint16_t CAN_bms_status_ltc_state_get(void){
	// Extract 4-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word0 & 0x0078) >> 3) << 0;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_ltc_error_count_get(void){
	// Extract 12-bit signal at bit offset 7
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word0 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word1 & 0x0007) >> 0) << 9;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_ltc_last_error_get(void){
	// Extract 8-bit signal at bit offset 19
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word1 & 0x07F8) >> 3) << 0;
	return (data * 1.0) + 0;
}
float CAN_bms_status_cpu_usage_percent_get(void){
	// Extract 8-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word2 & 0x0007) >> 0) << 5;
	return (data * 0.5) + 0;
}
float CAN_bms_status_cpu_peak_percent_get(void){
	// Extract 8-bit signal at bit offset 35
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word2 & 0x07F8) >> 3) << 0;
	return (data * 0.5) + 0;
}
float CAN_bms_status_vbus_voltage_get(void){
	// Extract 10-bit signal at bit offset 43
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word2 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word3 & 0x001F) >> 0) << 5;
	return (data * 0.1) + 0;
}
float CAN_bms_status_internal_temp_get(void){
	// Extract 8-bit signal at bit offset 53
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[2].word3 & 0x1FE0) >> 5) << 0;
	return (data * 1.0) + -40;
}
uint16_t CAN_bms_status_max_charge_current_mA_get(void){
	// Extract 16-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[3].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[3].word1 & 0x0007) >> 0) << 13;
	return (data * 1.0) + 0;
}
uint32_t CAN_bms_status_max_charge_voltage_mV_get(void){
	// Extract 18-bit signal at bit offset 19
	uint32_t data = 0;
	data |= (uint32_t)((CAN_bms_status_mux_payloads[3].word1 & 0xFFF8) >> 3) << 0;
	data |= (uint32_t)((CAN_bms_status_mux_payloads[3].word2 & 0x001F) >> 0) << 13;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_contactors_closed_get(void){
	// Extract 1-bit signal at bit offset 37
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[3].word2 & 0x0020) >> 5) << 0;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_precharge_active_get(void){
	// Extract 1-bit signal at bit offset 38
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[3].word2 & 0x0040) >> 6) << 0;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_charge_allowed_get(void){
	// Extract 1-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[3].word2 & 0x0080) >> 7) << 0;
	return (data * 1.0) + 0;
}
...
uint16_t CAN_bms_status_cell_B_balancing_get(void){
	...
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[4].word0 & 0x00F8) >> 3) << 0;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_cell_C_balancing_get(void){
	// Extract 5-bit signal at bit offset 8
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[4].word0 & 0x1F00) >> 8) << 0;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_cell_D_balancing_get(void){
	// Extract 5-bit signal at bit offset 13
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[4].word0 & 0xE000) >> 13) << 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[4].word1 & 0x0003) >> 0) << 3;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_status_cell_E_balancing_get(void){
	// Extract 5-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_status_mux_payloads[4].word1 & 0x007C) >> 2) << 0;
	return (data * 1.0) + 0;
}

// ISR callback for mux message demultiplexing
static void CAN_bms_status_rx_callback(CAN_message_S* msg) {
	// Extract mux value from received message
	uint16_t mux_value = get_bits((size_t*)msg->payload, CAN_BMS_STATUS_MULTIPLEX_OFFSET, CAN_BMS_STATUS_MULTIPLEX_RANGE);
	
	// Copy to appropriate mux buffer
	if (mux_value < 5) {
		// Copy payload to mux-specific buffer
		CAN_bms_status_mux_payloads[mux_value] = *msg->payload;
		// Update mux message metadata
		CAN_bms_status_mux[mux_value].last_received_timestamp = msg->last_received_timestamp;
		*CAN_bms_status_mux[mux_value].canMessageStatus = 1;  // Mark unread
	}
}
...
static CAN_payload_S CAN_bms_debug_mux_payloads[3] __attribute__((aligned(sizeof(CAN_payload_S))));
static volatile uint8_t CAN_bms_debug_mux_status[3] = {0};
static CAN_message_S CAN_bms_debug_mux[3];
...
uint8_t CAN_bms_debug_checkMuxUnread(uint8_t mux_index){
	if (mux_index >= 3) return 0;
	return CAN_checkDataIsUnread(&CAN_bms_debug_mux[mux_index]);
}
uint8_t CAN_bms_debug_checkMuxStale(uint8_t mux_index){
	if (mux_index >= 3) return 1;
	return CAN_checkDataIsStale(&CAN_bms_debug_mux[mux_index], 20);
}
...
float CAN_bms_debug_task_1ms_cpu_percent_get(void){
	...
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word0 & 0x03FC) >> 2) << 0;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_10ms_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 10
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word0 & 0xFC00) >> 10) << 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word1 & 0x0003) >> 0) << 6;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_100ms_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word1 & 0x03FC) >> 2) << 0;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_1000ms_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 26
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word1 & 0xFC00) >> 10) << 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word2 & 0x0003) >> 0) << 6;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_1ms_peak_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 34
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word2 & 0x03FC) >> 2) << 0;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_10ms_peak_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 42
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word2 & 0xFC00) >> 10) << 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word3 & 0x0003) >> 0) << 6;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_100ms_peak_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 50
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[0].word3 & 0x03FC) >> 2) << 0;
	return (data * 1) + 0;
}
float CAN_bms_debug_task_1000ms_peak_cpu_percent_get(void){
	// Extract 8-bit signal at bit offset 2
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word0 & 0x03FC) >> 2) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_debug_bool0_get(void){
	// Extract 1-bit signal at bit offset 10
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word0 & 0x0400) >> 10) << 0;
	return (data * 1.0) + 0;
}
uint16_t CAN_bms_debug_bool1_get(void){
	// Extract 1-bit signal at bit offset 11
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word0 & 0x0800) >> 11) << 0;
	return (data * 1.0) + 0;
}
float CAN_bms_debug_float1_get(void){
	// Extract 16-bit signal at bit offset 12
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word0 & 0xF000) >> 12) << 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word1 & 0x0FFF) >> 0) << 4;
	return (data * 0.01) + 0;
}
float CAN_bms_debug_float2_get(void){
	// Extract 16-bit signal at bit offset 28
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word1 & 0xF000) >> 12) << 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word2 & 0x0FFF) >> 0) << 4;
	return (data * 0.01) + 0;
}
uint16_t CAN_bms_debug_word1_get(void){
	// Extract 16-bit signal at bit offset 44
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word2 & 0xF000) >> 12) << 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[1].word3 & 0x0FFF) >> 0) << 4;
	return (data * 1) + 0;
}
uint16_t CAN_bms_debug_byte1_get(void){
	// Extract 8-bit signal at bit offset 2
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_debug_mux_payloads[2].word0 & 0x03FC) >> 2) << 0;
	return (data * 1) + 0;
}

// ISR callback for mux message demultiplexing
static void CAN_bms_debug_rx_callback(CAN_message_S* msg) {
	// Extract mux value from received message
	uint16_t mux_value = get_bits((size_t*)msg->payload, CAN_BMS_DEBUG_MULTIPLEX_OFFSET, CAN_BMS_DEBUG_MULTIPLEX_RANGE);
	
	// Copy to appropriate mux buffer
	if (mux_value < 3) {
		// Copy payload to mux-specific buffer
		CAN_bms_debug_mux_payloads[mux_value] = *msg->payload;
		// Update mux message metadata
		CAN_bms_debug_mux[mux_value].last_received_timestamp = msg->last_received_timestamp;
		*CAN_bms_debug_mux[mux_value].canMessageStatus = 1;  // Mark unread
	}
}

static CAN_payload_S CAN_bms_cell_voltages_mux_payloads[6] __attribute__((aligned(sizeof(CAN_payload_S))));
static volatile uint8_t CAN_bms_cell_voltages_mux_status[6] = {0};
static CAN_message_S CAN_bms_cell_voltages_mux[6];
...
uint8_t CAN_bms_cell_voltages_checkMuxUnread(uint8_t mux_index){
	if (mux_index >= 6) return 0;
	return CAN_checkDataIsUnread(&CAN_bms_cell_voltages_mux[mux_index]);
}
uint8_t CAN_bms_cell_voltages_checkMuxStale(uint8_t mux_index){
	if (mux_index >= 6) return 1;
	return CAN_checkDataIsStale(&CAN_bms_cell_voltages_mux[mux_index], 200);
}
...
uint16_t CAN_bms_cell_voltages_cell_1_voltage_get(void){
	...
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[0].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[0].word1 & 0x0003) >> 0) << 13;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_2_voltage_get(void){
	// Extract 15-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[0].word1 & 0xFFFC) >> 2) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[0].word2 & 0x0001) >> 0) << 14;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_3_voltage_get(void){
	// Extract 15-bit signal at bit offset 33
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[0].word2 & 0xFFFE) >> 1) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_4_voltage_get(void){
	// Extract 15-bit signal at bit offset 48
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[0].word3 & 0x7FFF) >> 0) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_5_voltage_get(void){
	// Extract 15-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[1].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[1].word1 & 0x0003) >> 0) << 13;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_6_voltage_get(void){
	// Extract 15-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[1].word1 & 0xFFFC) >> 2) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[1].word2 & 0x0001) >> 0) << 14;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_7_voltage_get(void){
	// Extract 15-bit signal at bit offset 33
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[1].word2 & 0xFFFE) >> 1) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_8_voltage_get(void){
	// Extract 15-bit signal at bit offset 48
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[1].word3 & 0x7FFF) >> 0) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_9_voltage_get(void){
	// Extract 15-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[2].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[2].word1 & 0x0003) >> 0) << 13;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_10_voltage_get(void){
	// Extract 15-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[2].word1 & 0xFFFC) >> 2) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[2].word2 & 0x0001) >> 0) << 14;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_11_voltage_get(void){
	// Extract 15-bit signal at bit offset 33
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[2].word2 & 0xFFFE) >> 1) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_12_voltage_get(void){
	// Extract 15-bit signal at bit offset 48
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[2].word3 & 0x7FFF) >> 0) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_13_voltage_get(void){
	// Extract 15-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[3].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[3].word1 & 0x0003) >> 0) << 13;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_14_voltage_get(void){
	// Extract 15-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[3].word1 & 0xFFFC) >> 2) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[3].word2 & 0x0001) >> 0) << 14;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_15_voltage_get(void){
	// Extract 15-bit signal at bit offset 33
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[3].word2 & 0xFFFE) >> 1) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_16_voltage_get(void){
	// Extract 15-bit signal at bit offset 48
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[3].word3 & 0x7FFF) >> 0) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_17_voltage_get(void){
	// Extract 15-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[4].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[4].word1 & 0x0003) >> 0) << 13;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_18_voltage_get(void){
	// Extract 15-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[4].word1 & 0xFFFC) >> 2) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[4].word2 & 0x0001) >> 0) << 14;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_19_voltage_get(void){
	// Extract 15-bit signal at bit offset 33
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[4].word2 & 0xFFFE) >> 1) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_20_voltage_get(void){
	// Extract 15-bit signal at bit offset 48
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[4].word3 & 0x7FFF) >> 0) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_21_voltage_get(void){
	// Extract 15-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[5].word0 & 0xFFF8) >> 3) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[5].word1 & 0x0003) >> 0) << 13;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_22_voltage_get(void){
	// Extract 15-bit signal at bit offset 18
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[5].word1 & 0xFFFC) >> 2) << 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[5].word2 & 0x0001) >> 0) << 14;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_23_voltage_get(void){
	// Extract 15-bit signal at bit offset 33
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[5].word2 & 0xFFFE) >> 1) << 0;
	return (data * 1) + 0;
}
uint16_t CAN_bms_cell_voltages_cell_24_voltage_get(void){
	// Extract 15-bit signal at bit offset 48
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_voltages_mux_payloads[5].word3 & 0x7FFF) >> 0) << 0;
	return (data * 1) + 0;
}

// ISR callback for mux message demultiplexing
static void CAN_bms_cell_voltages_rx_callback(CAN_message_S* msg) {
	// Extract mux value from received message
	uint16_t mux_value = get_bits((size_t*)msg->payload, CAN_BMS_CELL_VOLTAGES_MULTIPLEX_OFFSET, CAN_BMS_CELL_VOLTAGES_MULTIPLEX_RANGE);
	
	// Copy to appropriate mux buffer
	if (mux_value < 6) {
		// Copy payload to mux-specific buffer
		CAN_bms_cell_voltages_mux_payloads[mux_value] = *msg->payload;
		// Update mux message metadata
		CAN_bms_cell_voltages_mux[mux_value].last_received_timestamp = msg->last_received_timestamp;
		*CAN_bms_cell_voltages_mux[mux_value].canMessageStatus = 1;  // Mark unread
	}
}

static CAN_payload_S CAN_bms_cell_temperatures_mux_payloads[7] __attribute__((aligned(sizeof(CAN_payload_S))));
static volatile uint8_t CAN_bms_cell_temperatures_mux_status[7] = {0};
static CAN_message_S CAN_bms_cell_temperatures_mux[7];
...
uint8_t CAN_bms_cell_temperatures_checkMuxUnread(uint8_t mux_index){
	if (mux_index >= 7) return 0;
	return CAN_checkDataIsUnread(&CAN_bms_cell_temperatures_mux[mux_index]);
}
uint8_t CAN_bms_cell_temperatures_checkMuxStale(uint8_t mux_index){
	if (mux_index >= 7) return 1;
	return CAN_checkDataIsStale(&CAN_bms_cell_temperatures_mux[mux_index], 2000);
}
...
float CAN_bms_cell_temperatures_stack_1_LTC_internal_temp_get(void){
	...
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_stack_1_balance_temp_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_stack_2_LTC_internal_temp_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_stack_2_balance_temp_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[0].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_1_get(void){
	// Extract 12-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_2_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + 0;
}
float CAN_bms_cell_temperatures_temp_3_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_4_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[1].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_5_get(void){
	// Extract 12-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_6_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_7_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_8_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[2].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_9_get(void){
	// Extract 12-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_10_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_11_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_12_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[3].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_13_get(void){
	// Extract 12-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_14_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_15_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_16_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[4].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_17_get(void){
	// Extract 12-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_18_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_19_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_20_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[5].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_21_get(void){
	// Extract 12-bit signal at bit offset 3
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word0 & 0x7FF8) >> 3) << 0;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_22_get(void){
	// Extract 12-bit signal at bit offset 15
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word0 & 0x8000) >> 15) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word1 & 0x07FF) >> 0) << 1;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_23_get(void){
	// Extract 12-bit signal at bit offset 27
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word1 & 0xF800) >> 11) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word2 & 0x007F) >> 0) << 5;
	return (data * 0.1) + -40;
}
float CAN_bms_cell_temperatures_temp_24_get(void){
	// Extract 12-bit signal at bit offset 39
	uint16_t data = 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word2 & 0xFF80) >> 7) << 0;
	data |= (uint16_t)((CAN_bms_cell_temperatures_mux_payloads[6].word3 & 0x0007) >> 0) << 9;
	return (data * 0.1) + -40;
}

// ISR callback for mux message demultiplexing
static void CAN_bms_cell_temperatures_rx_callback(CAN_message_S* msg) {
	// Extract mux value from received message
	uint16_t mux_value = get_bits((size_t*)msg->payload, CAN_BMS_CELL_TEMPERATURES_MULTIPLEX_OFFSET, CAN_BMS_CELL_TEMPERATURES_MULTIPLEX_RANGE);
	
	// Copy to appropriate mux buffer
	if (mux_value < 7) {
		// Copy payload to mux-specific buffer
		CAN_bms_cell_temperatures_mux_payloads[mux_value] = *msg->payload;
		// Update mux message metadata
		CAN_bms_cell_temperatures_mux[mux_value].last_received_timestamp = msg->last_received_timestamp;
		*CAN_bms_cell_temperatures_mux[mux_value].canMessageStatus = 1;  // Mark unread
	}
}
...
void CAN_DBC_init(void) {
	...
	for (int i = 0; i < 5; i++) {
		CAN_bms_status_mux[i].payload = &CAN_bms_status_mux_payloads[i];
		CAN_bms_status_mux[i].canMessageStatus = &CAN_bms_status_mux_status[i];
	}
	// Register ISR callback for mux demultiplexing
	CAN_bms_status.rx_callback = CAN_bms_status_rx_callback;
	CAN_configureMailbox(&CAN_bms_status);
	CAN_configureMailbox(&CAN_bms_power_systems);
	// Initialize RX multiplexed message: debug
	for (int i = 0; i < 3; i++) {
		CAN_bms_debug_mux[i].payload = &CAN_bms_debug_mux_payloads[i];
		CAN_bms_debug_mux[i].canMessageStatus = &CAN_bms_debug_mux_status[i];
	}
	// Register ISR callback for mux demultiplexing
	CAN_bms_debug.rx_callback = CAN_bms_debug_rx_callback;
	CAN_configureMailbox(&CAN_bms_debug);
	// Initialize RX multiplexed message: cell_voltages
	for (int i = 0; i < 6; i++) {
		CAN_bms_cell_voltages_mux[i].payload = &CAN_bms_cell_voltages_mux_payloads[i];
		CAN_bms_cell_voltages_mux[i].canMessageStatus = &CAN_bms_cell_voltages_mux_status[i];
	}
	// Register ISR callback for mux demultiplexing
	CAN_bms_cell_voltages.rx_callback = CAN_bms_cell_voltages_rx_callback;
	CAN_configureMailbox(&CAN_bms_cell_voltages);
	// Initialize RX multiplexed message: cell_temperatures
	for (int i = 0; i < 7; i++) {
		CAN_bms_cell_temperatures_mux[i].payload = &CAN_bms_cell_temperatures_mux_payloads[i];
		CAN_bms_cell_temperatures_mux[i].canMessageStatus = &CAN_bms_cell_temperatures_mux_status[i];
	}
	// Register ISR callback for mux demultiplexing
